...
export function normalizeTicketInput(value: string) {
...
export function isLegacyTicket(value: string) {
  return value in legacyTicketLabels;
}

export function isValidExactTicket(value: string) {
...
export function formatPromptTicket(value: string) {
  if (isLegacyTicket(value)) return legacyTicketLabels[value];
...
export function isPromptQuestionAnswered(q: PromptQuestion, data: PromptBaseAnswers) {
  ...
  if (q.type === "currency") return isValidExactTicket(String(data[q.key]));
