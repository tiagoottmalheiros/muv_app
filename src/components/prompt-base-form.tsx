...
export function PromptBaseForm() {
  ...
  const [answers, setAnswers] = useState<PromptBaseAnswers>(appData.promptBase.answers);
  ...
  if (step === 0) return <section className="card mx-auto max-w-3xl p-6 md:p-9"><p className="eyebrow">Base estratégica</p><h2 className="text-2xl font-bold text-white">Explique seu negócio para a Central</h2><p className="mt-4 leading-7 text-muted">Clareza gera estratégia. Suas respostas serão registradas e usadas automaticamente para personalizar as próximas etapas.</p><div className="mt-7 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold text-muted">Seu nome<input className="field mt-2" value={answers.name} onChange={(e) => setField("name", e.target.value)} /></label><label className="text-xs font-bold text-muted">Seu e-mail<input className="field mt-2" type="email" value={answers.email} onChange={(e) => setField("email", e.target.value)} /></label><label className="text-xs font-bold text-muted sm:col-span-2">Nome do seu negócio<input className="field mt-2" value={answers.businessName} onChange={(e) => setField("businessName", e.target.value)} /></label></div><Button className="mt-7 w-full sm:w-auto" disabled={!promptBaseSchema.safeParse(answers).success} onClick={() => setStep(1)}>Começar Prompt Base<ArrowRight size={16} /></Button></section>;
