...
export function Brand({ compact = false }: { compact?: boolean }) {
  return <div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg border border-primary/40 bg-primary/10 text-[11px] font-black tracking-[.12em] text-primary">MUV</div>{!compact && <div><strong className="block text-sm tracking-[.2em] text-white">MUV</strong><span className="text-xs text-muted">Instalação guiada</span></div>}</div>;
}
...
export function VideoLesson({ title }: { title: string }) {
  return <div className="group relative aspect-video overflow-hidden rounded-[18px] border border-white/10 bg-[#050816]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_65%_30%,rgba(34,211,238,.12),transparent_40%)]" /><div className="absolute inset-0 grid place-items-center"><button aria-label={`Reproduzir ${title}`} className="grid size-16 place-items-center rounded-full border border-gold/40 bg-gold/10 text-gold transition group-hover:scale-105 group-hover:bg-gold group-hover:text-[#020617]"><Play size={24} fill="currentColor" /></button></div><div className="absolute bottom-0 left-0 right-0 flex items-end justify-between bg-gradient-to-t from-[#020617] p-5 pt-16"><div><span className="text-xs uppercase tracking-[.16em] text-gold">Vídeo da etapa</span><p className="mt-1 text-sm text-white">{title}</p></div><span className="text-xs text-muted">Conteúdo configurável</span></div></div>;
}
