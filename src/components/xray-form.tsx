...
export function XrayForm() {
  ...
  const score = answers.reduce((total, answer) => total + answer.score, 0); const result = getXrayResult(score); const bottleneck = getMainBottleneck(answers); const generated = generateXrayText(data.user.name, data.promptBase.answers.businessName, answers);
  ...
  if (step === 0) return <section className="mx-auto max-w-3xl space-y-5"><VideoLesson title="Raio-X Anti-Curiosos: Como diagnosticar seu funil" /><div className="card p-7 md:p-10"><p className="eyebrow">Diagnóstico MUV Starter</p><h2 className="text-2xl font-bold text-white">Descubra onde seu funil está vazando</h2><p className="mt-4 leading-7 text-muted">Assista à preparação e responda 12 perguntas para receber a classificação, a pontuação e o gargalo principal do seu funil. A Base Estratégica será usada como contexto nas próximas etapas.</p><div className="mt-7 grid grid-cols-3 gap-3">{[["12", "perguntas"], ["36", "pontos"], ["5", "classificações"]].map(([value, label]) => <div className="rounded-xl border border-white/8 p-4 text-center" key={label}><strong className="block text-2xl font-bold text-gold">{value}</strong><span className="text-[10px] uppercase tracking-wider text-muted">{label}</span></div>)}</div><Button className="mt-7 w-full sm:w-auto" onClick={() => setStep(1)}>Começar Raio-X<ArrowRight size={16} /></Button></div></section>;
